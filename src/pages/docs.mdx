...
title: Docs
description: Everything beyond install — build, configure, internals, troubleshooting, contributing. Install and FAQ live on the home page.
...
There's no `-dev` channel for the universal tarball — [build from source](#build-from-source) for that. To leave the dev channel, remove the `-dev` repo (or the `-git` package) and switch to the stable Copr / PPA / AUR.
...
- **Fedora:** see [packages/rpm/appgrid.spec.in](https://github.com/xarbit/plasma6-applet-appgrid/blob/main/packages/rpm/appgrid.spec.in) (`BuildRequires:` + `Requires:`)
- **Ubuntu:** see [packages/deb/debian/control](https://github.com/xarbit/plasma6-applet-appgrid/blob/main/packages/deb/debian/control) (`Build-Depends:` + `Depends:`)
...
- AppGrid does **not** share favorites with Kickoff or Kicker — those use their own client id (`org.kde.plasma.favorites`).
...
- `~/.local/lib/qt6/plugins/plasma/applets/dev.xarbit.appgrid.so` + `…appgrid.panel.so` — the compiled plasmoid plugins. As of 1.9 the QML interface and metadata are compiled **into** each `.so`; there is no loose `plasmoids/` QML directory anymore.
...
- `test_frecency_scoring.cpp` — KActivities frecency tiebreak in search ranking
- `test_sort.cpp` — sort modes (alphabetical, most used, by category)
- `test_category_mapping.cpp` — built-in category grouping
- `test_pluginhelpers.cpp` — KRunner plugin/favorites helpers
- `test_discoverbackends.cpp` — "Manage in Discover" backend resolution
...
**Iterating on the UI:** because the QML is compiled into the `.so`, `plasmoidviewer -a dev.xarbit.appgrid` can't load it from a loose package. Rebuild and restart plasmashell instead (`cmake --build build && sudo cmake --install build && kquitapp6 plasmashell && kstart plasmashell`). The QtQuickTest suite (see [Running the test suite](#running-the-test-suite)) is the fastest way to exercise QML logic without a full session.
...
- **Space:** [`#appgrid-plasma:matrix.org`](https://matrix.to/#/#appgrid-plasma:matrix.org) — joining the space adds you to both public rooms below.
- [**#appgrid-general**](https://matrix.to/#/#appgrid-general:matrix.org) — help, install issues, feature ideas.
- [**#appgrid-random**](https://matrix.to/#/#appgrid-random:matrix.org) — off-topic, KDE setups, screenshots.
- [**#appgrid-development**](https://matrix.to/#/#appgrid-development:matrix.org) — invite-only; open a PR or active issue first, then ping in #appgrid-general for access.

Don't have a Matrix account? Sign up free at [element.io](https://element.io/) (web/desktop/mobile) or any homeserver — federation means you can join from anywhere.
